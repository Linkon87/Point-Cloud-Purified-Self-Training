...
def build_model():
    ...
    print('Building PCT...')
    pct = PCT(num_classes=15).cuda()
    classifier = pct.model.linear3
    pct.model.linear3 = nn.Identity()
    net = ExtractorHead(pct, classifier)
    return net, pct, classifier


def load_model(net, args):
    filename = 'pointdp/pretrained/scanobjectnn/pct/best_checkpoint.pth'
