...
def build_model():
    ...
    print('Building Pointnet2...')
    pointnet2 = PointNet2(num_class=15).cuda()
    classifier = pointnet2.model.FC_layer
    pointnet2.model.FC_layer = nn.Identity()
    net = ExtractorHead(pointnet2, classifier)
    return net, pointnet2, classifier


def load_model(net, args):
    filename = 'pointdp/pretrained/scanobjectnn/pointnet2/best_checkpoint.pth'
    ...
    for k, v in net_dict.items():
        if k[:15] == "module.FC_layer":
            k = k.replace("module.FC_layer", "head")
        else:
