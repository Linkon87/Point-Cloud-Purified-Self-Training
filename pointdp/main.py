...
from  load_ae_model import load_model_opt_sched
...
def check_out_fmt(task, out, dataset_name):
    if task == 'cls':
        ...
        assert set(out.keys()) == {'logit', 'trans_feat','recons'}
        logit = out['logit']
...
def attack(task, loader, model, dataset_name, method, confusion=False):
    ...
    if method.METHOD == 'advpc':
        model_ae = models.Autoencoder(
            ...
            decoder='mlp')
        ##### HARDCODE #########################
        checkpoint = torch.load('./runs/point_ae_1/model_best_test.pth')
        ########################################
    ...
    for i, data_batch in enumerate(loader):
        ...
        with torch.no_grad():
            ...
            if confusion:
                if dataset_name in ['modelnet40_diffusion']:
                    attacked.append(
...
def train(task, loader, model, optimizer, loss_name, dataset_name, cfg):
    ...
    for i, data_batch in enumerate(loader):
        ...
        inp = get_inp(task, model, data_batch, loader.dataset.batch_proc, dataset_name)
        if task not in ['ae', 'diffusion', 'advpc']:
            if dataset_name == 'modelnet40_diffusion':
...
def entry_train(cfg, resume=False, model_path="",load_model_name=''):
    ...
    if resume:
        model = load_model_opt_sched(model, optimizer, lr_sched, bnm_sched, model_path,load_model_name)
...
def entry_test(cfg, test_or_valid, model_path="", confusion=False,load_model_name=''):
    ...
    if confusion:
        ...
        #### some hardcoding #######
        np.save('./output_ori/' + cfg.EXP.EXP_ID + '_recon.npy', recons)
        np.save('./output_ori/' + cfg.EXP.EXP_ID + '_pred.npy', pred)
        np.save('./output_ori/' + cfg.EXP.EXP_ID + '_ground.npy', ground)
        #### #### #### #### #### ####
    else:
...
def entry_attack(cfg, model_path="", confusion=False,load_model_name=''):
    # split = "test" if test_or_valid else "valid"
    loader_test = create_dataloader(split='test', cfg=cfg)
...
parser.add_argument('--exp-config', type=str, default='./configs/attack/sde/dgcnn_truncate_diffusion_pgd_sde.yaml')#'./configs/attack/dgcnn_truncate_diffusion_pgd_naive.yaml')#default='configs/dgcnn_si_adv.yaml')#default ='configs/attack/si_adv_attack/dgcnn_truncate_diffusion_si-adv.yaml')#
#")
...
parser.add_argument("--DATALOADER_num_workers", type=int, default=-1, help="Number of workers for the data loader")

parser.add_argument("--load_model_name", type=str, default='', choice=['PCT','Curvenet','dgcnn','pointmlp','pointnet','pointnet2',''],help='pretrained backbone to load')
