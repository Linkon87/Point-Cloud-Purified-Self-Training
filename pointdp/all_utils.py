...
from collections.abc import MutableMapping, Hashable #python3.10 更改
import csv
import os
import torch
...
DATASET_NUM_CLASS = {
    'ScanObjectNN':15,
    'ScanObjectNN_diffusion':15,
    'modelnet40': 40,
    'modelnet40_c': 40,
    'modelnet40_rscnn': 40,
    'modelnet40_pn2': 40,
    'modelnet40_dgcnn': 40,
    'modelnet40_diffusion': 40,
    'modelnet40_c_diffusion': 40,
    'modelnet40_customized': 40,
    'modelnet40_customized_diffusion': 40,
    'modelnet40_noise': 40
}
...
class PerfTrackVal:
    ...
    def __init__(self, task, extra_param=None):
        self.task = task
        if task in ['cls', 'cls_trans','attack_cls','non_adaptive_attack_cls','sde_attack_cls','si_adv_attack_cls']:
            ...
            assert False
    def update(self, data_batch, out, loss = None):
        if self.task in ['cls', 'cls_trans','attack_cls','non_adaptive_attack_cls','sde_attack_cls','si_adv_attack_cls']:
        ...
        elif self.task in ['ae','diffusion','advpc']:
            # print(loss)
            assert loss is not None
            self.all.append(loss)
        else:
            assert False
    def agg(self):
        if self.task in ['cls', 'cls_trans','attack_cls','non_adaptive_attack_cls','sde_attack_cls','si_adv_attack_cls']:
            ...
            perf = {
                'chamfer' : - sum(self.all) / len(self.all)
            }
        else:
            assert False
        return perf
...
class PerfTrackTrain(PerfTrackVal):
    ...
    def update_all(self, data_batch, out, loss):
        self.update(data_batch, out, loss)
...
class SORDefense(torch.nn.Module):
