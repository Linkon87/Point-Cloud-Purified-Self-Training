...
def main():
    ...
    parser.add_argument('--test_data', type=str,
                        default='scanobjectnn/make_data/pct/adaptive/4changing/changing_inf_batch_attack.npy')
    parser.add_argument('--test_label', type=str,
                        default='scanobjectnn/make_data/pct/adaptive/4changing/label.npy')
    ...
    for te_batch_idx, (te_inputs, te_labels) in enumerate(te_dataloader):
        ...
        with torch.no_grad():
            ...
            advpc_idx = ((te_inputs_idx // 64) % 5) == 0
            auto_idx = ((te_inputs_idx // 64) % 5 )== 1
            pgd_idx = ((te_inputs_idx // 64) % 5) == 2
            sde_idx = ((te_inputs_idx // 64) % 5) == 4
            clean_idx =((te_inputs_idx // 64) % 5 )== 3

            correct_advpc.append(predicted[advpc_idx].cpu().eq(te_labels[advpc_idx]))
            correct_auto.append(predicted[auto_idx].cpu().eq(te_labels[auto_idx]))
            ...
            correct_sde.append(predicted[sde_idx].cpu().eq(te_labels[sde_idx]))
    ...
    print('auto : {:.2f}'.format(torch.cat(correct_auto).numpy().mean() * 100))
    print('pgd : {:.2f}'.format(torch.cat(correct_pgd).numpy().mean() * 100))
    print('sde : {:.2f}'.format(torch.cat(correct_sde).numpy().mean() * 100))
    print('Mix : {:.2f}'.format(torch.cat(correct).numpy().mean() * 100))
    print('---------------')

    del correct ,correct_clean, correct_auto,correct_advpc,correct_pgd,correct_sde
    ###########  #################
    ...
    for te_batch_idx, (te_inputs, te_labels) in enumerate(te_dataloader):
        ...
        for iter_id in range(0, min(args.iters, int(len(mini_batch_indices) / 128) + 1) + 2):
            ...
            for batch_idx, (inputs, labels) in enumerate(tr_extra_dataloader):
                ...
                if args.is_fix:
                    ...
                    pro_u, pseudo_label_u = softmax_logit.max(dim=1)  # fixmatch without ema
        ...
        with torch.no_grad():
            ...
            print('real time auto : {:.2f}'.format(torch.cat(correct_auto).numpy().mean() * 100))
            print('real time pgd : {:.2f}'.format(torch.cat(correct_pgd).numpy().mean() * 100))
            print('real time sde : {:.2f}'.format(torch.cat(correct_sde).numpy().mean() * 100))
    ...
    toc = time.time()
    print('total time (min):', (toc - tic) / 60)
