...
def main():
    ...
    parser.add_argument('--test_data', type=str,
                        default='scanobjectnn/make_data/pointnet2/4changing/changing_inf_batch_attack.npy')
    parser.add_argument('--test_label', type=str,
                        default='scanobjectnn/make_data/pointnet2/4changing/label.npy')

    parser.add_argument('--backbone', default='pointnet2', type=str,choices=['pointnet2','pct','curvenet'])
    ...
    for te_batch_idx, (te_inputs, te_labels) in enumerate(te_dataloader):
        ...
        with torch.no_grad():
            ...
            cw_idx = ((te_inputs_idx // 64) % 5 )== 1
            pgd_idx = ((te_inputs_idx // 64) % 5) == 2
            knn_idx = ((te_inputs_idx // 64) % 5) == 4
            ...
            correct_knn.append(predicted[knn_idx].cpu().eq(te_labels[knn_idx]))
    ...
    print('knn : {:.2f}'.format(torch.cat(correct_knn).numpy().mean() * 100))
    print('Mix : {:.2f}'.format(torch.cat(correct).numpy().mean() * 100))

    print('---------------')


    del correct ,correct_clean, correct_cw,correct_advpc,correct_pgd,correct_knn
    ###########  #################
    ...
    for te_batch_idx, (te_inputs, te_labels) in enumerate(te_dataloader):
        ...
        with torch.no_grad():
            ...
            print('real time knn : {:.2f}'.format(torch.cat(correct_knn).numpy().mean() * 100))
