...
parser.add_argument('--data_root', type=str, default='',
                    help='Path to point cloud npy file.')
parser.add_argument('--label_root', type=str, default='',
                    help='Path to point cloud label npy file.')
...
def defend_npz_test_data(path=args.data_root,label=args.label_root):
    '''Apply defense to all pc in a npz file.
    Then save it for evaluation.
    '''
    # load data
    # npz = np.load(path)
    test_pc = np.load(path)
    test_label = np.load(label)
    # try:
...
def defend_npz_train_test_data(path=args.data_root,label=args.label_root):
    '''Apply defense to all pc in a npz file.
    Then save it for evaluation.
    '''
    # load data
...
if __name__ == '__main__':
    if args.train:
        ...
        defend_npz_train_test_data(args.data_root,args.label_root)
    else:
        ...
        if os.path.isdir(data_root):
            ...
            for file in all_files:
                ...
                if os.path.isfile(one_file):
                    defend_npz_test_data(one_file,args.label_root)
        else:
            defend_npz_test_data(args.data_root,args.label_root)
