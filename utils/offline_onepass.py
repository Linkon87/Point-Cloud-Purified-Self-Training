...
def offline(trloader, ext, num_classes=40):
    if os.path.exists('offline_onepass.pth'):
        data = torch.load('offline_onepass.pth')
        return data
    ...
    torch.save((feat_mean, feat_cov, feat_mean_categories, feat_cov_categories), 'offline_onepass.pth')
